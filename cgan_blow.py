...
if __name__ == "__main__":
    ...
    print(
        f"    Deep Learning by Conditional Generative Adversarial Networks      {now}"
    )
    print(f"")
    ...
    for kep, (inp, rlt) in trdata.repeat().take(step + 1).enumerate():
        ...
        if kep != 0 and (kep) % 10 == 0:
            ...
            now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            print(
                f"  Step {kep}   dt {crt-pvt:.2f} sec   total time {crt-init:.2f} sec       {now}"
            )
            print(
                f"  Step {kep}   dt {crt-pvt:.2f} sec   total time {crt-init:.2f} sec       {now}",
                file=o,
    ...
    print(
        f"  Elapsed time     {eptime:.2f} seconds  ( {hour} hour  {minute} minutes  {second} seconds )"
    )
    print(f"")
